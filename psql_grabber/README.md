...
Run `npm install` to get all necesarry packages to run the node application.
...
Currently, since the pure focus of this application is to recieve psql queries to get TV's available from a specified database, there is no routing for 'GET' requests. Only a routing for POST requests are handled.
...
`ProcessSearch()`: Takes a JSON object, and checks if all necesarry fields are both present and valid. Returns `true` if valid, and `ERROR: FIELD_NAME` upon failure of verifying a particular field.

`isValidBrand()`: Takes a brand string and verifies directly with the database if it's a valid value.

`isValidTechnology()`: Analgous to `isValidBrand()`.

`isValidResolution()`: Analgous to `isValidBrand()`.
